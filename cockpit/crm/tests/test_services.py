...
def test_entity_upsert_idempotent():
    uid = uuid.uuid4()
    e1 = scd2_upsert_entity(uid, 'PERSON', 'Alice')
    assert e1.is_current is True
    assert Entity.objects.count() == 1
    e2 = scd2_upsert_entity(uid, 'PERSON', 'Alice')
    assert Entity.objects.count() == 1
    assert e1.id == e2.id
    e3 = scd2_upsert_entity(uid, 'PERSON', 'Alice B')
...
def test_detail_upsert_idempotent():
    uid = uuid.uuid4()
    scd2_upsert_entity(uid, 'PERSON', 'Bob')
    d1 = scd2_upsert_detail(uid, 'EMAIL', {'value': "bob@example.com"})
    assert d1.is_current
    assert EntityDetail.objects.count() == 1
    d2 = scd2_upsert_detail(uid, 'EMAIL', {'value': "bob@example.com"})
    assert EntityDetail.objects.count() == 1
    assert d1.id == d2.id
    d3 = scd2_upsert_detail(uid, 'EMAIL', {'value': "bob.new@example.com"})
